...
| 83 | **Every Gemini error was mislabelled "out of quota"** | the quota regex `rate` matched "generateContent" in the request URL, so even a 403 project-denied read as quota; and a 403 was never surfaced as an account problem | quota regex word-bounded; a 403/PERMISSION_DENIED is reported as an account error with the fix, checked first; `replay` 33 |
| 94 | **The final check "waited for Gemini" forever — actually a missing-thumbnail crash misread as an outage** | the saved render skipped the thumbnail silently (`.catch(()=>{})`); resume built a path to it; the review read it and threw ENOENT; the catch labelled that "vision unavailable" and deferred | the save cuts a thumbnail from the video if the copy fails; resume regenerates a missing one; the review only sends images that EXIST; a non-provider error (ENOENT/parse/type) is re-thrown, never deferred as an outage; verified in isolation |
| 92 | **Groq vision failed on every 2nd-3rd scene, cascading to a dead Gemini 403** | the contact-sheet image is thousands of INPUT tokens; Groq's free ~6K tokens-per-minute cap was hit after a few calls, and the "tokens per minute" error was thrown as FATAL instead of retried | vision images downscaled to 768px; vision calls paced (`VISION_MIN_GAP_MS`); a per-minute limit is now a `RateLimitError` that waits and retries the SAME provider; `replay` 34 |
| 93 | OpenRouter 401 on every vision fallback | its text-model defaults pointed at a retired z-ai:free slug | dead defaults blanked; a fallback with no usable model for the call is skipped, not called |
| 91 | OpenRouter "free" vision models had moved to paid | the :free slugs I hardcoded were retired; OpenRouter returns a confusing 401/404 for them | the probe reads live pricing+modality from /models and picks a genuinely-free vision model; the test lists current free ones |
| 90 | OpenRouter 401 "User not found" with a valid key | not the request (headers are correct) — an expired key, a key from a different account, or free models needing training enabled | probe verifies the key against /auth/key first and prints the exact fix; headers already sent |
| 89 | Z.ai removed; OpenRouter 401 diagnosed | Z.ai's free vision tier ended (insufficient balance); OpenRouter returned 401 "User not found" = bad/empty key, and it needs HTTP-Referer/X-Title headers | Z.ai deleted; OpenRouter sends the required headers in both the client and the probe; a 401 now tells the person to refresh the key |
| 88 | **Groq accepted vision, then rejected it for token limit** | a vision call reserved a heavy-tier `max_tokens` (thousands); Groq's free per-minute output cap counts reserved tokens, so a 50-token ranking was refused as "tokens per minute" | image calls reserve `VISION_MAX_TOKENS` (1000); OpenRouter added as a second free vision provider so Groq is not alone; `select-test` proved Groq vision scores real footage |
| 86 | `vision:probe` rejected a WORKING vision model | its test image was a hand-typed 1x1 that is not a valid JPEG, so Groq's real vision model (qwen3.8-27b) returned "invalid image data" (400) and was discarded | the probe embeds a real ffmpeg-made JPEG and queries each provider's /models list; a 400 "invalid image" is no longer treated as "model unavailable" |
| 87 | Contact sheet sent to vision providers with a hardcoded image/jpeg mime | a PNG sheet would be mislabelled | the data-URL mime matches the file extension |
| 85 | `vision:probe` reported every key as missing | it ran as plain `node`, which does not read `.env`, so `process.env` was empty | the probe loads `.env` itself and the package script uses the app's env loader |
| 84 | Guessed vision model slugs 404'd (Groq Scout, Z.ai glm-4.5v) | slugs differ by account and move | `npm run vision:probe` finds the slug your key actually serves and writes it to .env; defaults updated to widely-available ones |
| 82 | **Groq vision was never actually reached** | `fallbackChain()` only knew OPENAI_COMPAT/LLM_FALLBACKS, so a NAMED provider like Groq was invisible; with LLM_ROLE_VISION unset the vision role stayed on Gemini and Groq was never tried | every configured NAMED provider joins the fallback chain, carrying its vision model; proven with the role unset in `replay` 32 |
| 79 | (feature) a real SECOND vision provider — Groq's free Qwen/Llama-4 vision — so footage selection survives a Gemini outage | — | `LLM_ROLE_VISION=groq`; image calls route only to models that can see; `replay` 30 |
| 80 | (feature) the channel could not produce at all while Gemini was down — the point of a video creator that cannot create | — | `visionOptional=true` builds the video from the stock libraries' own top-ranked footage, HELD for approval; `replay` 31 |
| 81 | (feature) changing the channel's topics meant hand-editing JSON | — | `node scripts/set-topics.mjs` — a validated list with the picker's rules written in |
| 77 | **Gemini down at the final check threw away a finished render** | the check fell back to "hold 0/10"; the repair loop then re-scripted and re-rendered a video nobody could judge; the upload gate rejected it; the runner was wiped | the render is saved as a workflow artifact and the video waits at `rendered`; the next run downloads it and resumes at the final check; mid-repair and repeat outages save again; an expired artifact falls back to re-rendering the kept script |
| 78 | Post-review logic would have been duplicated for the resume path | two copies of the upload gate would drift (registry pattern #1) | lifted into one `finishAfterReview`, diffed line by line against the original before use |
